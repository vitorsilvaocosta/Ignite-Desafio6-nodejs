...
describe("Create Statement Controller",()=>{
  ...
  beforeAll(async ()=>{
    ...
    id2 = uuidV4();
    password = await hash("testeT",8);
    await connection.query(
      `INSERT INTO users(id,name,email,password)
      VALUES('${id2}','testeT','testeT@testeT.com.br','${password}')`
    );
  })
  ...
  it("should be able to create a new statement transfer",async()=>{
    ...
    const response = await request(app).post(`/api/v1/statements/transfer/${id2}`)
    .send({
      amount: 80,
  ...
  afterAll(async()=>{
    ...
    await connection.query("DELETE FROM users WHERE name = 'testeT'");
